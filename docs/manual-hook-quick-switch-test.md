...
- NOTE: Blender file browser exposed `blender.exe/GHOST_WindowClass/Blender File View`, not a supported foreground `#32770` dialog; native hook diagnostics classify it as a custom file browser, and the app now routes it through the Blender custom-dialog adapter (`Ctrl+L`, target folder, `Enter`).
...
- Custom-dialog or nonstandard-picker candidates to treat as unsupported until proven otherwise: Bambu Suite, Corel apps, Autodesk Fusion, RPCS3, Raspberry Pi Imager, Rufus, ScreenToGif.
